...
class Order{
	...
	public:
		explicit Order(const Side& side, const Price& price, const OrderId& orderId, const OrderType& orderType, const Quantity& initialQuantity, const Quantity& remainingQuantity)
			: side_ { side }
			, price_ { price }
			, orderId_ { orderId }
			...
			, remainingQuantity_ { remainingQuantity } 
			{
				...
				if(remainingQuantity != initialQuantity){
                    throw std::invalid_argument(std::format("Order({}) must have remaining and initial quantity the same on init", orderId));
				}
		...
		[[nodiscard]] bool isFilled() const noexcept { return getFilledQuantity() == getInitialQuantity(); }
		void fill(const Quantity& quantity){
			...
			if(quantity == 0){
				throw std::invalid_argument(std::format("Quantity to fill cannot be less than 1 Order({})", getOrderId()));
			}
